...
test('GET templates returns defaults for all four states + generic name', async () => {
  const res = await request(getApp()).get('/gmail/customer-email/templates');
  expect(res.status).toBe(200);
  expect(Object.keys(res.body.templates).sort()).toEqual(['fulfilled', 'received', 'sent', 'shipped']);
